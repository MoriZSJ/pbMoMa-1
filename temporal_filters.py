...
class SlidingWindow (object):
    ...
    def process(self, data_itr):        # not used
    ...
    def update(self, data):
        if self.memory is None:
            ...
            # print("selfmemory: ",self.memory,self.memory.shape)
...
class IdealFilter (object):
    ...
    def __call__(self, data, axis=0):
        ...
        #### show fft frequency
        # freq = np.fft.fftfreq(fft.shape[1],d = 1./2)
        # i = np.argsort(freq)
        # freq = freq[i]
        # fft2 = abs(fft[-1][i])
        ...
        # pyplot.plot(freq,fft2)
        # x_tck = np.arange(min(freq), max(freq)+1,0.5)
        # pyplot.xticks(x_tck)
        # pyplot.title("fft")
        # pyplot.show()
